...
bindings/node/                     # napi-rs 3.x Node native addon (Session 34) — NOT in Cargo.toml include
├── Cargo.toml                    # napi = 3, napi-derive = 3 (no napi-build, no build.rs — napi 3.x handles Windows/delay-load at runtime)
├── package.json                  # @napi-rs/cli build, npm name `noos-regulator`, 5 prebuilt target triples
├── src/lib.rs                    # Parallel port of bindings/python: Regulator / LLMEvent / Decision / CircuitBreakReason / CorrectionPattern (napi-rs auto-generates .d.ts). Includes `Regulator.metricsSnapshot` + `LLMEvent.fromOtelSpanJson` + `withImplicitCorrectionWindowSecs`.
