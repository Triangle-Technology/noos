...
- **Session 37 Full-codebase principle-compliance sweep** (2026-04-19, user instruction "Kiểm tra toàn bộ Noos... review code if it against principles"). Doc-only cleanup, no behaviour changes. **Audit scope**: all 10 principles + CR1-CR5 against the full codebase, not just one session's delta (S29 / S35b were scoped to recent changes). **Clean**: P3 (no new duplication), P4 (types/ + math/ have no crate imports, cognition respects layer hierarchy), P5 (173 `unwrap()` calls all inside `#[cfg(test)]` blocks; production code clean; 2 documented candle-mmap `unsafe` exceptions in `inference/mamba.rs` with `// P5 exception:` comments), P6 (every cognition + regulator module has `#[cfg(test)]`; `types/*.rs` Layer 0 shapes have no logic to test), P7 (CLAUDE.md + memory current to S36). **9 fixes applied**: (1-2) **P1 regulator exemption preamble** — added `**Scope note (P1 / P9b)**` block to `src/regulator/tools.rs` (opaque tool-name sequence tracking) and `src/regulator/state.rs` (pure data-shape persistence). Previously flagged by S35b as "pre-existing, outside S30-35 scope". (3) **P2 `/// Mutable:` prefix** — `src/session.rs::inject_gate_feedback` had `Mutable:` text buried ~10 lines down; reordered to put `Mutable:` as the first doc line per §P2 ("must start their doc comment with `/// Mutable:`"). (4-9) **P10 Gating sections** — added `## Gating (P10)` blocks to 6 cognition modules that lacked them: `belief_state.rs` (state-update utility, no priority competition), `dynamics.rs` (regime classifier, output is modulation input for `adaptive_thresholds` / `resource_allocator`), `emotional.rs` (scalar producer consumed via `belief_state::update_affect`, gating deferred to `adaptive_thresholds`), `intervention.rs` (Tầng 1 sampling adapter, gating delegated upstream per §P10 single-source rule), `resource_allocator.rs` (application-infrastructure allocator, not a convergence-loop signal), `world_model.rs` (state maintenance via `perceive` / `consolidate` / `maintain`, invoked unconditionally at phase boundaries). Each section answers the 4 §P10 questions (fires when / inactive when / suppresses / suppressed by) with references to real symbols verified to exist. **Known-debt items deliberately NOT touched** (per documented scope decisions, not new violations): P9b lexicon regex in `emotional.rs` (NEGATIVE_HIGH/MOD + POSITIVE_HIGH/MOD English) + `detector.rs` (SEQUENCE_MARKERS English) remain as Path C scoped interim per `memory/project_lexicon_removal_analysis_2026_04_14.md`. **Doc-link cleanup same session**: introduced 5 rustdoc warnings during the Gating writes (bad intra-doc link to nonexistent `SamplingOverride::none`; 3 redundant explicit `(crate::path::Type)` link targets where the short `[Type]` form resolves; 1 redundant target on `CorrectionPattern`). All 5 fixed — `cargo doc --lib --no-deps` zero warnings. **Verification**: 463 tests pass (unchanged), 0 clippy warnings (the 3 "hard linking" warnings from cargo are Windows Sync FS + unrelated to code), 0 rustdoc warnings. No publish (doc-only, per the 2026-04-18 refined rule). **Principle status at end of Session 37**: clean across full codebase for the principles audited, modulo documented-interim items.
